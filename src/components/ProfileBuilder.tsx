...
const ProfileBuilder: React.FC<ProfileBuilderProps> = ({ 
  ...
  return (
    <div className="h-full bg-slate-800/30 overflow-y-auto">
      {/* Header with Progress */}
      <div className="p-6 border-b border-white/20 bg-slate-800/50">
      ...
      <div className="p-6 space-y-6">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className={`text-lg font-semibold ${
                        activeSection === section.key ? 'text-blue-300' : 'text-white'
                      }`}>
                        {section.title}
                      </h3>
                      {activeSection === section.key && (
                        <p className="text-xs text-blue-400 mt-1">Currently filling this section</p>
                      )}
                    </div>
                  </div>
                ...
                {section.fields.map((field, fieldIndex) => (
                  <div key={fieldIndex} className="flex items-start space-x-3">
                    <field.icon className={`w-4 h-4 flex-shrink-0 mt-1 ${
                      field.value ? 'text-purple-400' : 'text-gray-500'
                    }`} />
        ...
        {/* Completion Message */}
        {isCompleted && (
          <div className="bg-gradient-to-r from-green-500/20 to-blue-500/20 backdrop-blur-sm rounded-xl border border-green-500/50 p-6 text-center">
